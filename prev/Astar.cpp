...
void AStar_without_weather() {
    ...
    start_node->G = 0.0;
    start_node->H = geodesic(Start.first, Start.second, End.first, End.second);
    ...
    while (!open.empty()) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                double newG = curNode->G + geodesic(curNode->lat, curNode->lon, nbrNode->lat, nbrNode->lon);
                if (newG < nbrNode->G) {
                    nbrNode->G = newG;
                    nbrNode->H = geodesic(nbrNode->lat, nbrNode->lon, end_node->lat, end_node->lon);
                    nbrNode->F = nbrNode->G + nbrNode->H;
                    nbrNode->last = curNode;
                    open.push(make_pair(nbrNode, nbrNode->F));
                }
            }
        }
    }
